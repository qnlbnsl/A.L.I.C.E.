...
class Client:
    ...
    async def record_from_queue(
        self,
        queue: Queue,
        out_file="output_recording.wav",
    ):
        """
        Record audio data from the input stream and save it to a WAV file.
        ...
        try:
            for _ in range(0, int(self.rate / self.chunk * self.record_seconds)):
                ...
                self.frames += data.tobytes()

                audio_array = Client.bytes_to_float_array(data.tobytes())
...
class TranscriptionClient:
    ...
    async def __call__(self, queue: Queue, audio=None):
        """
        ...
        if audio is not None:
            ...
            await self.client.record_from_queue(queue=queue)
