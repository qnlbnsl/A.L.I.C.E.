...
def divide_audio_into_chunks(
    audio_chunk: NDArray[np.int16], original_chunk_size=640, channels=8
) -> List[NDArray[np.int16]]:
    # Calculate the size of each chunk (original_chunk_size * number of channels)
    chunk_size = original_chunk_size * channels

    # Divide the audio chunk into 3 equal parts
    # Each part has size of chunk_size
    return [audio_chunk[i * chunk_size : (i + 1) * chunk_size] for i in range(3)]
